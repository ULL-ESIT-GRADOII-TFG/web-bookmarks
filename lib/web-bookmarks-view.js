...
export default class RaulBookmarksView {

  constructor(serializedState) {
    // Create root element
    this.element = document.createElement('div');
    this.element.classList.add('raul-bookmarks');
    this.element.id = 'raul-bookmarks';
      ...
      if($('#raul-bookmarks').is(':visible')) {
        ...
        $('#btns-creados').click(function(e){
          if((e.target.tagName == 'BUTTON') && (e.target.value)){
            ...
            document.getElementById(id).addEventListener("click", element.openUrl(value));
          }
        });
  ...
  changeHeight() {
    ...
    for (var i = 0; i < changeDim.length; i++) {
      changeDim[i].addEventListener("click", function() {
        document.getElementById("btns-creados").style.height = $('#raul-bookmarks').height() - $('#container').height() - 27 + "px";
      });
    }
  }
  ...
  newBookmark() {
    ...
    if(folder) {
      ...
      //savebookmarks[folder][bookmark] = url;
    }
    else {
  ...
  serialize() {
    // return {
    //   deserializer: 'raul-bookmarks/RaulBookmarksView'
    // };
  }
  ...
  getTitle() {
    return 'Raul Bookmarks'
  }

  getURI() {
    return 'atom://raul-bookmarks'
  }
